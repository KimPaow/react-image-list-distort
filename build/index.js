...
/**
 * shape = 'circle', radius = 0.6, segments = 64, width = 1, height = 1, segmentsWidth = 32, segmentsHeight = 32
 ...
 * @prop {Object} options
 * @prop {Object<Number>} [options.strength=0.25] How powerful the distort is
 * @prop {Object<String>} [options.effect=''] A string defining what extra effect to apply. Defaults to "redshift", can also pass "stretch"
 * @prop {Object<String>} [options.shape='circle'] A string defining the shape of the geometry. Defaults to "circle", can also pass "plane". If circle then the image should be square.
 * @prop {Object<Number>} [options.radius=0.6] A number defining the radius(size) of the shape. Only applicable when shape is 'circle'
 * @prop {Object<Number>} [options.segments=64] Defines the number of segments of the shape when the shape is 'circle
 * @prop {Object<Number>} [options.width=1] Defines the width of the shape when the shape is 'plane'
 * @prop {Object<Number>} [options.height=1] Defines the height of the shape when the shape is 'plane'
 * @prop {Object<Number>} [options.segmentsWidth=32] Defines the number of segments on the X-axis of the shape when the shape is 'plane'
 * @prop {Object<Number>} [options.segmentsHeight=32] Defines the number of segments on the Y-axis of the shape when the shape is 'plane'
 */
...
class ImageDistort extends Component {
  ...
  onMouseMove(event) {
    ...
    let y = this.mouse.y.map(-1, 1, -this.viewSize.height / 2, this.viewSize.height / 2); // update plane position
  ...
  onPositionUpdate() {
    // compute offset
    let offset = this.mesh.position.clone().sub(this.position) // velocity
  ...
  onTargetChange(index) {
    ...
    if (!this.currentItem.texture) return; //update texture

    this.uniforms.uTexture.value = this.currentItem.texture; // compute image ratio

    let imageRatio = this.currentItem.img.naturalWidth / this.currentItem.img.naturalHeight; // scale plane to fit image dimensions
  ...
  setup() {
    ...
    this.camera.position.set(0, 0, 3); //ADD RENDERER
  ...
  getGeometry() {
    const {
      ...
      segmentsWidth = 32,
      segmentsHeight = 32
    } = this.props.options ? this.props.options.geometry ? this.props.options.geometry : {} : {};
  ...
  getListItems({
    ...
    const listItems = this.listWrapper ? [...this.listWrapper.querySelectorAll(this.props.itemRoot)] : [];
